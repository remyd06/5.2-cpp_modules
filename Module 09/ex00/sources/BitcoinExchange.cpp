...
/*   Updated: 2025/12/05 15:01:36 by rdedola          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Bitcoin::Bitcoin(const std::string &input)
{
	...
	// Store the parsed input.txt file in std::map container
	getline(file, buffer);
	while (getline(file, buffer))
		this->_map[buffer.substr(0, buffer.find(' '))]=buffer.substr(13);
	...
	// Iterators declaration for each std::map
	std::map<std::string, std::string>::iterator it = this->_map.begin();
	std::map<std::string, std::string>::iterator it_csv = this->_map_csv.begin();
		
	// Error handler for too early date
	if (it->first < "2009-01-02")
		throw (std::runtime_error("Error: the date must not be earlier than 2009-01-02"));
	if (it_csv->first < "2009-01-02")
		throw (std::runtime_error("Error.csv: the date must not be earlier than 2009-01-02"));
...
void	Bitcoin::parseInput(const std::string &input)
{
	...
	if (!file.is_open())
		throw (std::runtime_error("Error: the file could not be opened."));
